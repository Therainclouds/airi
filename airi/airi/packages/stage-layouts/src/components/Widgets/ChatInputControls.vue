...
const props = defineProps<{
  ...
  fileAccept: string
  attachmentCapabilityHint: string
  cameraSupported: boolean
  cameraActive: boolean
  cameraPanelOpen: boolean
  cameraStatus: 'idle' | 'requesting-permission' | 'previewing' | 'capturing' | 'denied' | 'error' | 'unsupported'
  cameraHasDraft: boolean
}>()
...
const cameraButtonTitle = computed(() => {
  if (props.cameraPanelOpen)
    return t('stage.camera.close')
  if (props.cameraStatus === 'requesting-permission')
    return t('stage.camera.requestingPermission')
  if (props.cameraStatus === 'denied')
    return t('stage.camera.permissionDenied')
  if (props.cameraStatus === 'error')
    return t('stage.camera.startFailed')
  if (props.cameraHasDraft)
    return t('stage.camera.draftReady')
  return t('stage.camera.open')
})
...
const cameraButtonClass = computed(() => {
  ...
  if (props.cameraPanelOpen || props.cameraActive || props.cameraHasDraft)
    return 'text-primary-500 dark:text-primary-300'
  return 'text-neutral-500 dark:text-neutral-400'
})

const cameraIconClass = computed(() => {
  if (props.cameraStatus === 'requesting-permission')
    return 'i-ph:spinner-gap animate-spin'
  if (props.cameraHasDraft)
    return 'i-ph:camera-plus-fill'
  if (props.cameraActive)
    return 'i-ph:camera-rotate-fill'
  return 'i-ph:camera'
})
...
<template>
  <div class="flex items-center gap-2 border-t border-primary-300/20 px-4 py-3 dark:border-primary-500/10">
    <input ref="fileInput" type="file" multiple :accept="fileAccept" class="hidden" @change="emit('filesSelected', $event)">
    <button
      ...
      :title="t('stage.attachments.action')"
    ...
    <button
      ...
      @click="emit('toggleCameraPanel')"
    >
      <div :class="cameraIconClass" class="h-5 w-5" />
    </button>
          ...
          >
            <div class="flex flex-col items-center justify-center">
              ...
              <p class="mt-2 text-center text-xs text-neutral-500 dark:text-neutral-400">
                {{ autoSendEnabled ? t('stage.microphone.autoSendReady') : t('stage.microphone.autoSendOff') }}
              </p>
            </div>
    ...
    <div class="max-w-[18rem] flex-1 truncate text-xs opacity-60">
      {{ attachmentCapabilityHint }}
    </div>
