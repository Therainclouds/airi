...
const showOpenClawPromptTemplateConfirm = ref(false)
const showOpenClawPromptWarningConfirm = ref(false)
const showOpenClawPromptSessionReminder = ref(false)
const openClawPromptWarningMatches = ref<string[]>([])
const pendingSaveCard = ref<Card | null>(null)
...
const recommendedOpenClawPromptTemplate = computed(() => t('settings.pages.card.creation.openclawprompt_template'))

function applyRecommendedOpenClawPromptTemplate() {
  openClawPrompt.value = recommendedOpenClawPromptTemplate.value.trim()
}

function requestApplyRecommendedOpenClawPromptTemplate() {
  if (!openClawPrompt.value.trim()) {
    applyRecommendedOpenClawPromptTemplate()
    return
  }

  showOpenClawPromptTemplateConfirm.value = true
}

function cancelApplyRecommendedOpenClawPromptTemplate() {
  showOpenClawPromptTemplateConfirm.value = false
}

function confirmApplyRecommendedOpenClawPromptTemplate() {
  applyRecommendedOpenClawPromptTemplate()
  showOpenClawPromptTemplateConfirm.value = false
}

function detectOpenClawPromptRiskMatches(value: string) {
  const normalizedValue = value.trim()
  const rules = [
    { label: 'ACT', pattern: /\bACT\b/i },
    { label: 'cognitive', pattern: /\bcognitive\b/i },
    { label: 'intent', pattern: /\bintent\b/i },
    { label: 'motion', pattern: /\bmotion\b/i },
    { label: 'DELAY', pattern: /\bDELAY\b/i },
    { label: 'JSON', pattern: /\bjson\b/i },
  ]

  return rules
    .filter(rule => rule.pattern.test(normalizedValue))
    .map(rule => rule.label)
}

const openClawPromptWarningDescription = computed(() => {
  const matches = openClawPromptWarningMatches.value.join(', ')
  return matches
    ? t('settings.pages.card.creation.openclawprompt_warning_description', { matches })
    : t('settings.pages.card.creation.openclawprompt_warning_description_fallback')
})
...
function saveCard(card: Card): boolean {
  ...
  const shouldShowSessionReminder = shouldRemindOpenClawPromptSessionRefresh(normalizedOpenClawPrompt)
  const riskMatches = detectOpenClawPromptRiskMatches(normalizedOpenClawPrompt)

  if (riskMatches.length > 0) {
    openClawPromptWarningMatches.value = riskMatches
    pendingSaveCard.value = rawCard
    showOpenClawPromptSessionReminder.value = shouldShowSessionReminder
    showOpenClawPromptWarningConfirm.value = true
    return false
  }

  persistCard(rawCard, existingAiriExtension, normalizedOpenClawPrompt, shouldShowSessionReminder)
  return true
}

function shouldRemindOpenClawPromptSessionRefresh(normalizedOpenClawPrompt: string) {
  if (!isEditMode.value || !props.cardId || !bridgeProviderIds.includes(selectedChatProviderId.value)) {
    return false
  }

  const existingCard = cardStore.getCard(props.cardId)
  const previousPrompt = existingCard?.extensions?.airi?.agents?.openclaw?.prompt?.trim() ?? ''
  return previousPrompt !== normalizedOpenClawPrompt
}

function persistCard(rawCard: Card, existingAiriExtension: AiriExtension | undefined, normalizedOpenClawPrompt: string, shouldShowSessionReminder = false) {
  ...
  if (shouldShowSessionReminder) {
    showOpenClawPromptSessionReminder.value = true
    return
  }
...
function cancelOpenClawPromptWarningConfirm() {
  showOpenClawPromptWarningConfirm.value = false
  pendingSaveCard.value = null
  openClawPromptWarningMatches.value = []
}

function confirmOpenClawPromptWarningConfirm() {
  const rawCard = pendingSaveCard.value
  if (!rawCard) {
    cancelOpenClawPromptWarningConfirm()
    return
  }

  const existingAiriExtension = rawCard.extensions?.airi as AiriExtension | undefined
  persistCard(rawCard, existingAiriExtension, openClawPrompt.value.trim(), showOpenClawPromptSessionReminder.value)
  cancelOpenClawPromptWarningConfirm()
}

function closeOpenClawPromptSessionReminder() {
  showOpenClawPromptSessionReminder.value = false
  modelValue.value = false
}
...
<template>
      ...
      <DialogContent class="fixed left-1/2 top-1/2 z-100 m-0 max-h-[90vh] max-w-6xl w-[92vw] flex flex-col overflow-auto border border-neutral-200 rounded-xl bg-white p-5 shadow-xl 2xl:w-[60vw] lg:w-[80vw] md:w-[85vw] xl:w-[70vw] -translate-x-1/2 -translate-y-1/2 data-[state=closed]:animate-contentHide data-[state=open]:animate-contentShow dark:border-neutral-700 dark:bg-neutral-800 sm:p-6">
        <div class="w-full flex flex-col gap-5">
          ...
          <div v-else-if="activeTab === 'settings'" class="tab-content ml-auto mr-auto w-95%">
            ...
            <div class="mx-auto mb-4 w-90% border border-neutral-200 rounded-xl bg-neutral-50 px-4 py-3 text-sm text-neutral-700 dark:border-neutral-700/60 dark:bg-neutral-900/40 dark:text-neutral-200">
              <div class="mb-2 font-medium">
                {{ t('settings.pages.card.creation.openclawprompt_guideline_title') }}
              </div>
              <ul class="list-disc pl-5 space-y-1">
                <li>{{ t('settings.pages.card.creation.openclawprompt_guideline_identity') }}</li>
                <li>{{ t('settings.pages.card.creation.openclawprompt_guideline_behavior') }}</li>
                <li>{{ t('settings.pages.card.creation.openclawprompt_guideline_safety') }}</li>
                <li>{{ t('settings.pages.card.creation.openclawprompt_guideline_output') }}</li>
              </ul>
              <div class="mt-3 flex justify-end">
                <Button
                  variant="secondary"
                  icon="i-solar:clipboard-text-bold-duotone"
                  :label="t('settings.pages.card.creation.openclawprompt_apply_template')"
                  @click="requestApplyRecommendedOpenClawPromptTemplate"
                />
              </div>
            </div>
            <div class="input-list ml-auto mr-auto w-90% flex flex-row flex-wrap justify-center gap-8">
  ...
  <AlertDialogRoot :open="showOpenClawPromptTemplateConfirm" @update:open="showOpenClawPromptTemplateConfirm = $event">
    <AlertDialogPortal>
      <AlertDialogOverlay class="fixed inset-0 z-110 bg-black/50 data-[state=closed]:animate-fadeOut data-[state=open]:animate-fadeIn" />
      <AlertDialogContent class="fixed left-1/2 top-1/2 z-110 max-w-md w-full border border-neutral-200 rounded-xl bg-white p-6 shadow-xl -translate-x-1/2 -translate-y-1/2 data-[state=closed]:animate-contentHide data-[state=open]:animate-contentShow dark:border-neutral-700 dark:bg-neutral-800">
        <AlertDialogTitle class="mb-4 text-xl font-normal">
          {{ t('settings.pages.card.creation.openclawprompt_confirm_title') }}
        </AlertDialogTitle>
        <AlertDialogDescription class="mb-6 whitespace-pre-line">
          {{ t('settings.pages.card.creation.openclawprompt_confirm_description') }}
        </AlertDialogDescription>
        <div class="flex flex-row justify-end gap-3">
          <AlertDialogCancel as-child>
            <Button
              variant="secondary"
              :label="t('settings.pages.card.cancel')"
              @click="cancelApplyRecommendedOpenClawPromptTemplate"
            />
          </AlertDialogCancel>
          <AlertDialogAction as-child>
            <Button
              variant="primary"
              :label="t('settings.pages.card.creation.openclawprompt_confirm_apply')"
              @click="confirmApplyRecommendedOpenClawPromptTemplate"
            />
          </AlertDialogAction>
        </div>
      </AlertDialogContent>
  ...
  <AlertDialogRoot :open="showOpenClawPromptWarningConfirm" @update:open="showOpenClawPromptWarningConfirm = $event">
    <AlertDialogPortal>
      <AlertDialogOverlay class="fixed inset-0 z-110 bg-black/50 data-[state=closed]:animate-fadeOut data-[state=open]:animate-fadeIn" />
      <AlertDialogContent class="fixed left-1/2 top-1/2 z-110 max-w-md w-full border border-amber-200 rounded-xl bg-white p-6 shadow-xl -translate-x-1/2 -translate-y-1/2 data-[state=closed]:animate-contentHide data-[state=open]:animate-contentShow dark:border-amber-800/60 dark:bg-neutral-800">
        <AlertDialogTitle class="mb-4 text-xl font-normal">
          {{ t('settings.pages.card.creation.openclawprompt_warning_title') }}
        </AlertDialogTitle>
        <AlertDialogDescription class="mb-6 whitespace-pre-line">
          {{ openClawPromptWarningDescription }}
        </AlertDialogDescription>
        <div class="flex flex-row justify-end gap-3">
          <AlertDialogCancel as-child>
            <Button
              variant="secondary"
              :label="t('settings.pages.card.cancel')"
              @click="cancelOpenClawPromptWarningConfirm"
            />
          </AlertDialogCancel>
          <AlertDialogAction as-child>
            <Button
              variant="primary"
              :label="t('settings.pages.card.creation.openclawprompt_warning_continue')"
              @click="confirmOpenClawPromptWarningConfirm"
            />
          </AlertDialogAction>
        </div>
      </AlertDialogContent>
  ...
  <AlertDialogRoot :open="showOpenClawPromptSessionReminder" @update:open="showOpenClawPromptSessionReminder = $event">
    <AlertDialogPortal>
      <AlertDialogOverlay class="fixed inset-0 z-110 bg-black/50 data-[state=closed]:animate-fadeOut data-[state=open]:animate-fadeIn" />
      <AlertDialogContent class="fixed left-1/2 top-1/2 z-110 max-w-md w-full border border-primary-200 rounded-xl bg-white p-6 shadow-xl -translate-x-1/2 -translate-y-1/2 data-[state=closed]:animate-contentHide data-[state=open]:animate-contentShow dark:border-primary-800/60 dark:bg-neutral-800">
        <AlertDialogTitle class="mb-4 text-xl font-normal">
          {{ t('settings.pages.card.creation.openclawprompt_session_reminder_title') }}
        </AlertDialogTitle>
        <AlertDialogDescription class="mb-6 whitespace-pre-line">
          {{ t('settings.pages.card.creation.openclawprompt_session_reminder_description') }}
        </AlertDialogDescription>
        <div class="flex flex-row justify-end gap-3">
          <AlertDialogAction as-child>
            <Button
              variant="primary"
              :label="t('settings.pages.card.creation.openclawprompt_session_reminder_confirm')"
              @click="closeOpenClawPromptSessionReminder"
            />
          </AlertDialogAction>
        </div>
      </AlertDialogContent>
